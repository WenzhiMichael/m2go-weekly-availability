...
export default function ScheduleApp() {
  ...
  return (
    <main className="app-shell employee-page">
      ...
      <section className="dashboard-head employee-head"><div><p className="eyebrow">员工个人页面</p><h1>你好，<br /><em>{employee.displayName}</em></h1></div><aside className="week-summary"><span>NEXT WEEK</span><strong>{shortDate(weekStart)} — {dates[6] && shortDate(dates[6])}</strong><p>正在收集下周时间</p></aside></section>
      ...
      {activeView === "availability" ? <section className="surface availability-surface">
        <div className="section-heading"><div><p className="step-label">下周可上班时间</p><h2>{employee.displayName}，请选择每一天</h2></div><div className={`save-state ${status}`}><i />{status === "saving" ? "自动保存中…" : status === "saved" ? "已自动保存" : status === "error" ? "保存失败" : "修改后自动保存"}</div></div>
      ...
      <footer className="site-footer"><strong>M2GO by Mandarin</strong></footer>
...
function PublishedSchedule({ schedule, activeEmployeeId, label, pending = false }: { schedule: PublishedWeek | null; activeEmployeeId: number; label: string; pending?: boolean }) {
  if (!schedule?.publishedAt) return pending ? <section className="surface pending-schedule"><div className="section-heading"><div><p className="step-label">{label}</p><h2>经理正在安排中</h2></div><span className="count-badge muted">尚未发布</span></div></section> : null;
  const dates = Array.from({ length: 7 }, (_, index) => addDays(schedule.weekStart, index));
...
function AccessPage() {
  ...
  return <main className="auth-shell"><BrandHeader subtitle="STAFF ACCESS" actions={<Link className="ghost-action" href="/manager">经理入口</Link>} />{setupMode ? <section className="auth-layout"><div className="auth-copy"><p className="eyebrow">第一次使用／重置 PIN</p><h1>设置你的<br /><em>四位 PIN。</em></h1><p>{setupEmployee ? `你好，${setupEmployee.displayName}。这个 PIN 由你自己记住，经理只能重置，不能查看。` : "正在确认你的设置链接。"}</p></div><form className="pin-card" onSubmit={setupPin}><span className="auth-kicker">STAFF PIN</span><h2>{setupEmployee ? `${setupEmployee.displayName}，设置 PIN` : "等待链接确认"}</h2><label>输入四位数字<input aria-label="设置四位员工 PIN" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} type="password" value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="••••" required /></label><label>再输入一次<input aria-label="确认四位员工 PIN" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} type="password" value={confirmPin} onChange={(event) => setConfirmPin(event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="••••" required /></label><button disabled={!setupEmployee || pin.length !== 4 || confirmPin.length !== 4 || busy}>{busy ? "设置中…" : "设置并进入我的页面"}</button>{error && <p className="form-error" role="alert">{error}</p>}<small>链接设置成功后会自动失效。</small></form></section> : <section className="staff-auth-layout"><div className="auth-copy"><p className="eyebrow">员工入口</p><h1>选择名字，<br /><em>进入我的时间表。</em></h1>{invalid && <p className="form-error">这个设置链接无效或已经使用，请联系经理重新发送。</p>}</div>{selected ? <form className="pin-card staff-pin-card" onSubmit={login}><button type="button" className="back-choice" onClick={() => { setSelected(null); setPin(""); setError(""); }}>← 重新选择名字</button><span className="auth-kicker">EMPLOYEE #{String(selected.id).padStart(2, "0")}</span><h2>{selected.displayName}</h2><label>四位员工 PIN<input aria-label={`${selected.displayName} 四位员工 PIN`} inputMode="numeric" pattern="[0-9]{4}" maxLength={4} type="password" value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="••••" required /></label><button disabled={pin.length !== 4 || busy}>{busy ? "登录中…" : "进入我的时间表"}</button>{error && <p className="form-error" role="alert">{error}</p>}<small>忘记 PIN？请联系经理重新发送设置链接。</small></form> : <div className="staff-name-picker">{employees.map((person) => <button key={person.id} onClick={() => { setSelected(person); setError(""); }}><span>#{String(person.id).padStart(2, "0")}</span><strong>{person.displayName}</strong><small>这是我 →</small></button>)}{error && <p className="form-error" role="alert">{error}</p>}</div>}</section>}</main>;
}
